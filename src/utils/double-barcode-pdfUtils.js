...
export const getDoubleBarcodeDoc = (combine, listLength, docList, doc, item, services, shipperDetails, partnerName) => {
  console.log("getDoubleBarcodeDoc")
  ...
  doc.setFontType('bold');
  doc.setFontSize(8);
  doc.text(`For ${partnerName}`, 41, 125);
...
export const getDropItDoc = (combine, listLength, docList, doc, item, services, shipperDetails) => {
  console.log("getDropItDoc")
  let trackingNo = item.tracking_no;
  let pickupAddress = item.pickup_address;
  let pickupCountry = item.pickup_country;
  let shipperName = shipperDetails.shipper_name;
